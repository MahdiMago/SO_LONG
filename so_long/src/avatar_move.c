...
/*   Updated: 2025/02/01 16:11:07 by mamagoma         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int avatar_move(t_data *data, t_vars *vars, Position *pos, int direction)
{
    ...
    if (vars->lines[pos->y][pos->x] == 'E' && data->mapinfo.count_collectibles == pos->c_count)
    {
        ft_printf("!!!!!!!!!    SUCCESS    !!!!!!!!!\n");
		exit_game(data);
...
{
    int new_y = data->pos.y + (direction == 1 ? -1 : 1);
...
{
    int new_x = data->pos.x + (direction == 3 ? 1 : -1);
